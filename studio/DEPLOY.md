...
## ⚠️ Все образы — из GHCR (сервер в РФ не тянет docker.io)

Сервер Timeweb MSK ловит **429 Too Many Requests** при анонимном pull с Docker Hub
(`docker.io`). Поэтому не только приложение, но и **nginx** и **certbot** берутся
из GHCR: workflow копирует их в `ghcr.io/kirill-design367/aurea-nginx` и
`…/aurea-certbot` **внутри Actions** (там лимитов нет), а сервер тянет всё из
одного реестра. docker.io на сервере не участвует вообще — в том числе при
авто-продлении сертификата (оно работает на уже скачанном образе). Аварийный
откат на docker.io — через `NGINX_IMAGE`/`CERTBOT_IMAGE` в `.env` (см. раздел **В**).
...
| `GHCR_TOKEN` | PAT c правом **`read:packages`** — сервер тянет **приватный** образ из GHCR |

**Про образ и `GHCR_TOKEN`.** Пакет `aurea-web` в GHCR — **приватный** (по умолчанию),
поэтому сервер логинится в `ghcr.io` этим токеном перед `pull`. Так ничего лишнего
не публикуется, и первый деплой проходит сразу (публичный пакет потребовал бы
ручного переключения видимости и правки workflow — усложнение, не упрощение).

Как создать `GHCR_TOKEN`: GitHub → **Settings → Developer settings → Personal
access tokens**:
- **Tokens (classic)** — область (scope) **`read:packages`**; либо
- **Fine-grained** — доступ к этому репозиторию, разрешение **Packages: Read-only**.

Выставьте срок действия **подлиннее (или без истечения)** — иначе, когда токен
протухнет, автодеплой начнёт падать на шаге `pull` (в разделе **В** описано, как
это распознать и обновить токен). Сборку и публикацию образа в Actions делает
встроенный `GITHUB_TOKEN` (в workflow задан `permissions: packages: write`) —
его заводить не нужно.

> Хотите **публичный** образ (без `GHCR_TOKEN` на сервере)? После первого билда
> откройте пакет → **Package settings → Change visibility → Public**, уберите из
> `deploy.yml` строку `docker login … GHCR_TOKEN` и повторно запустите деплой.
> По умолчанию оставляем приватным — так проще на старте.
...
**Признак протухшего `GHCR_TOKEN`:** в логе Actions шаг деплоя падает на
`docker login`/`docker compose pull` с `401 Unauthorized` или `denied`.
Лечение: создать новый PAT (`read:packages`) и обновить секрет `GHCR_TOKEN`
(Settings → Secrets and variables → Actions), затем Re-run workflow.

### В2a. Ошибка `429 Too Many Requests` (лимиты docker.io)
**Признак:** в логе — `429 Too Many Requests` / `toomanyrequests` при pull
`nginx:1.27-alpine` или `certbot/certbot` с `registry-1.docker.io`.

По умолчанию сервер сюда не ходит — nginx и certbot зеркалируются в GHCR (шаг
*Mirror nginx & certbot to GHCR* в workflow), и `deploy-remote.sh` тянет их
оттуда. Если 429 всё же виден:

- **На сервере** (внезапно тянется docker.io — например, кто-то переопределил
  образ): убедитесь, что в `.env` **нет** строк `NGINX_IMAGE=`/`CERTBOT_IMAGE=`,
  указывающих на docker.io (по умолчанию берётся GHCR-зеркало), и повторите деплой.
- **В Actions** (429 на шаге зеркалирования — редко, у раннера свой лимит):
  добавьте секреты `DOCKERHUB_USER` и `DOCKERHUB_TOKEN` (бесплатный аккаунт
  Docker Hub → Account → Personal access tokens, `Read-only`) — workflow сам
  залогинится в docker.io **только внутри Actions** и поднимет лимит. Затем
  Re-run workflow.
- **Крайний случай** (нужен docker.io прямо с сервера): временно
  `NGINX_IMAGE=nginx:1.27-alpine` / `CERTBOT_IMAGE=certbot/certbot` в `.env` и
  повторять `docker compose pull` с интервалом (лимит анонимно сбрасывается за 6 ч).

### В2b. `unbound variable` / `.env` устарел после обновления
**Признак:** в логе деплоя `deploy-remote.sh: line NN: XXX: unbound variable`
или жалоба, что в `.env` нет какой-то переменной.

Причина класса: `.env` на сервере не в git (и правильно — там пути/оверрайды),
поэтому `git reset --hard` его не обновляет, а скрипт работает под `set -u`.
Устранено системно:
- `deploy-remote.sh` и `bootstrap.sh` задают **безопасные дефолты** для всех
  своих переменных (`: "${VAR:=…}"`), так что пустой/старый `.env` их не роняет;
- в начале деплоя скрипт **сам сверяет `.env` с `.env.example`** и дописывает
  недостающие ключи их дефолтами (в логе — строки `.env: нет 'KEY' — дописываю…`).

Если всё же увидели такое (например, сервер не подтянул свежий скрипт): 
```bash
ssh -i ~/.ssh/aurea_deploy deploy@104.171.137.47
cd /opt/aurea && git pull            # взять актуальные скрипты
cd studio && bash scripts/deploy-remote.sh   # .env дособерётся сам
```
Ошибка **в самом скрипте** (не «сайт не отвечает») теперь перехватывается и
**НЕ вызывает авто-откат** — контейнеры не трогаются, в логе явное сообщение
«внутренняя ошибка скрипта… авто-откат НЕ выполняю».

**Кэш next/image переживает передеплой:** named volume `next-cache` смонтирован в
`/app/.next/cache` (оптимизированные картинки + ISR не пересобираются каждый деплой).
**Логи при провале health-check теперь печатаются в Actions автоматически**
(`docker compose logs --tail=50 web` + nginx перед откатом) — не нужен ручной заход.
