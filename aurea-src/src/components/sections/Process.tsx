...
export default function Process() {
  ...
  return (
    <section ref={sectionRef} id="process" className="section-padding relative overflow-hidden" style={{ background: 'var(--bg-primary)' }}>
      <GoldenMark variant="spiral-tl" size={650} className="absolute -top-20 -left-20" />
      <GoldenMark variant="circles-bl" size={440} className="absolute -bottom-10 right-0" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
            {steps.map((step, i) => {
              ...
              return (
                ...
                >
                  <div
                    className="relative p-7 rounded-2xl transition-all duration-500 group"
                    style={{ background: 'var(--surface)' }}
                  >
                    {/* Step number */}
                    <div className="absolute -top-4 left-7">
                      <span className="font-syne font-bold text-xs tracking-[0.2em] text-text-muted px-3 py-1 rounded-full" style={{ background: 'var(--bg-secondary)' }}>
                        {step.num}
                      </span>
                    </div>

                    {/* Icon */}
                    <div className={`step-icon-${i} flex items-center justify-center w-12 h-12 rounded-xl mb-5`} style={{ background: 'var(--surface-hover)' }}>
